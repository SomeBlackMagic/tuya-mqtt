...
import {TuyaCommand, SubDevice, TuyaEvent} from './types';
...
export class TuyaDevice {
  ...
  private readonly deviceDriver: BaseDeviceDefinition;
  ...
  constructor(
      protected readonly config: LocalDeviceConfig,
    ...
    this.connectionHandler = new DeviceConnectionHandler({
      ...
      ip: config.ip ?? null,
      version: config.protocolVersion ?? '3.1',
  ...
  private setupHandlerCallbacks(): void {
    ...
    this.stateHandler.setStateUpdatedCallback((updatedKeys: string[]) => {
      ...
      this.onStateChangedCallback?.(this.config.deviceId, updatedKeys);
    });
  }
  ...
  async onConnected(): Promise<void> {
    if (this.connectionHandler.isConnected()) {
      ...
      this.onDeviceConnectedCallback?.(this.config.deviceId);
  ...
  async onDisconnected(): Promise<void> {
    ...
    this.onDeviceDisconnectedCallback?.(this.config.deviceId);
    await Utils.sleep(5);
  ...
  onData(data: TuyaEvent): void {
    if (typeof data === 'object') {
      ...
      if (data.cid) {
        debug('Received JSON data from device ' + this.config.deviceId + ' for cid: ' + data.cid + ' ->', JSON.stringify(data.dps));
        ...
        debug(
          'Received JSON data from device ' + this.config.deviceId + ' ->',
      ...
      if (data !== 'json obj data invalid') {
        debug(
          'Received string data from device ' + this.config.deviceId + ' ->',
  ...
  getDeviceDriver(): BaseDeviceDefinition | null {
  ...
  async publishState(): Promise<void> {
    ...
    this.logger.debug(
      `Device state: ${this.config.name || this.config.deviceId} - ${JSON.stringify(state)}`,
    );
